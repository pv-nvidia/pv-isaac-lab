...
class RendererCfg:
    ...
    Only the OVRTX renderer implements this; other renderers warn and render synchronously. On the
    OVRTX ovstage scene-ownership path, every frame's scene writes drain the renders still in
    flight, so that path sustains at most one frame of latency — larger values are clamped to ``1``
    there with a warning. :data:`ASYNC_RENDERING_ENV_VAR` overrides this value.
    """
...
def async_rendering_enabled_from_env() -> bool | None:
    """Return whether :data:`ASYNC_RENDERING_ENV_VAR` enables asynchronous rendering.

    Kept as the boolean view over :func:`async_rendering_frames_from_env` for callers that only
    need the flag.

    Returns:
        The override flag, or ``None`` when the variable is unset, empty, or unparsable.
    """
    frames = async_rendering_frames_from_env()
...
def resolve_async_rendering_enabled(cfg: RendererCfg) -> bool:
    ...
    The boolean view over :func:`resolve_async_rendering_frames`.
    ...
    """
    return resolve_async_rendering_frames(cfg) > 0
